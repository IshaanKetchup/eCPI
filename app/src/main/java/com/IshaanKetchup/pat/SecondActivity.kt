...
class SecondActivity : AppCompatActivity() {
    ...
    private lateinit var textViewClimb: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navigationView.setCheckedItem(R.id.nav_second_activity)  // In SecondActivity
        ...
        navigationView.setCheckedItem(R.id.nav_second_activity)
        ...
        textViewClimb = findViewById(R.id.textViewClimb)
    ...
    private fun getPressureAltitude(): Double? {
        ...
        return if (qnh != null && elevation != null) {
            calculatePressureAltitude(qnh, elevation) // Replace with your actual method
        } else {
            null
        }
    }
    ...
    fun interpolatePressure(pressureAltitude: Double): Double {
        ...
        val b = pressureMap[baseAlt] ?: return Double.NaN
        val bNext = pressureMap[nextAlt] ?: return Double.NaN
        val bPrime = b - bNext
    ...
    fun calculatePressureAltitude(elevationFt: Double, qnhMb: Double): Double {
        ...
        val c = conversionMap[nearest] ?: return Double.NaN
    ...
    private fun calculateAndDisplay() {
        try{
            ...
            //STEP FUNCTION FOR MAX POWER CALCULATION
            val stepCount = (densityAltitude / 660.0).toInt()
            var maxPowerAvailable = 0.8 + (stepCount * 0.01)
            if(maxPowerAvailable>1){
                maxPowerAvailable=1.0
            }

            //CLIMB POWER CALCULATION
            var climb = 0.75 + (stepCount * 0.01)
            if(climb>1){
                climb = 1.0
            }
            ...
            var maxWeight =  (maxPowerAvailable * pa * 288.0 * 6600.0) / (1013.0 * (temperature + 273.0))
            if(maxWeight>4850){
                maxWeight = 4850.0
            }
            ...
            //CRUISE POWER
            val cp = 0.8
            textViewClimb.text = "%.2f".format(climb)
            textViewDensityAltitude.text = "%.2f ft".format(densityAltitude)
            textViewMaxPower.text = "%.2f".format(maxPowerAvailable)
            textViewCruisePower.text = "%.2f".format(cp)

            textViewMaxWeight.text = if (givenWeight != null && maxWeight < givenWeight) {
                "%.2f lbs - NOT POSSIBLE".format(maxWeight)
            } else {
                "%.2f lbs".format(maxWeight)
            }

            if (givenWeight != null) {
                //POWER TO HOVER
                val powerToHover = (givenWeight * 1013.0 * (temperature + 273.0)) / (pa * 288.0 * 6600)
                textViewHover.text =
